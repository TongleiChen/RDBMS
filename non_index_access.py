...
def INSERT(db_system:System,insert_parser:INSERT_tree_Evaluator):
    ...
    if db_system.INDEX == True:
        db_system.insert_data(insert_parser.table_name,insert_parser.insert_cols,insert_parser.insert_vals)
    else:
        db_system.insert_data_no_index(insert_parser.table_name,insert_parser.insert_cols,insert_parser.insert_vals)
...
if __name__=='__main__':
    ...
    if OPTION == "INIT":
        test_system = System()
        test_system.init_database("DEMO_NO_INDEX")
    else: # load
        test_system = load_database("DEMO_NO_INDEX")
        recover(test_system)
    
    test_system.TREE_OPTIMIZER = True
    test_system.JOIN_OPTIMIZER = True
    test_system.INDEX = False
    ...
    print("Insert time: ",round(time.time()-start_time,5))
